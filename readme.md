...
![Flowchart](research/assets/flowchart-V1.png)
...
**Orchestration Trigger:** AWS EventBridge Scheduler
...
<h3 style="color:#00a2ed">🔮 Serverless Inference Layer (Predictor)</h3>

**Compute:** AWS Lambda (`agro-price-forecasting`) orchestrated automatically via the Silver layer.
**Engine:** `ai_edge_litert` (TensorFlow Lite runtime optimized for serverless execution environments).

**Key Optimizations:**
* **Zero-TensorFlow Footprint:** Bypasses the 250 MB AWS Lambda layer storage ceiling by executing the entire multi-step projection cycle using native **NumPy** matrix manipulations instead of full TensorFlow computational graph allocation.
* **On-the-Fly Forward Normalization:** Intercepts raw incoming Parquet values from the Silver layer, safely transforms features to a `[0, 1]` state space using lightweight parameters stored in S3 (`saved_scalers/`), executes the models, and broad-scales predictions back to true market values.
...
## 🖥️ Local Visualization Dashboard (Streamlit)

The system includes a locally running Streamlit analytics dashboard that establishes a live connection with your AWS S3 data lake to fetch model binaries, scalers, and rolling predictions.

![Dashboard Overview UI](research/assets/dashboard_preview.png)

### Key Analytical Views:
1. **Dynamic Metric Evaluation Matrix:** Live-calculates the 30-day directional market outlook deltas (both numerical value and percentage momentum shift) comparing the most recent daily asset close price against the model endpoints.
2. **Hybrid Timeline Charting (Plotly):** Provides an interactive time-series overlay connecting historical spot prices to the 30-day rolling recursive forecasts generated by both the **M7** (short-term) and **M30** (longer context) model architectures.
3. **Macro-Timeline Range Toggles:** Native viewport adjustments.
...
```bash
python scripts/upload_models_to_aws.py
```

### Part 4: Launching the Local Analytics UI
To review your live production forecasts and compare multi-model outputs on your local machine, run the following commands from the project root:
...
The compilation engine will automatically open your default browser interface pointing to http://localhost:8501.


## 🚀 Future Roadmap: Pipeline Iterations

* **Trading Business-Day Calendar Constraints:** Refactor the current rolling 30-step NumPy matrix concatenation from standard calendar days to a strict business-day filter to handle weekend market gaps.
* **Automated Concept-Drift Monitoring:** Set up automated Slack/Discord notification webhooks inside the Gold layer triggered when daily price variations break historical variance bounds by more than 3 standard deviations.
